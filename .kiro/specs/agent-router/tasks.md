...
- [ ] 3. Configuration loading and validation (`config.ts`)
  - [ ] 3.1 Implement `resolveEnvValues` — recursively walk a config object and replace `ENV:X` string values with `process.env[X]`, throwing `FatalError` if the env var is unset
