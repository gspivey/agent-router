...
- [x] 1. Add daemon token store and HTTP endpoint
  - [x] 1.1 Create `src/daemon-token.ts`
  ...
  - [x] 1.2 Add `POST /hooks/event` route in `src/server.ts`
  ...
  - [x] 1.3 Wire token store and endpoint in `src/index.ts`
  ...
  - [x] 1.4 Unit test `src/daemon-token.ts`
  ...
  - [x] 1.5 Tier 2 test for `/hooks/event`
...
- [x] 2. Implement `verifySession` core
  - [x] 2.1 Create `src/verify-session.ts`
  ...
  - [x] 2.2 Wire `verifySession` into `cli-server.complete_session` (CRITICAL — contract preservation)
    ...
    - If `result.verified === false && result.reason === 'prs_still_open'` → the `open_prs` field on the result carries the list of still-open PRs directly; return `{ error: '<existing message>', open_prs: result.open_prs }` in the **exact shape** the prior `OpenPRsError` branch produced. The existing tier2 tests in `merge-pr.test.ts` assert this shape and MUST pass without modification.
  ...
  - [x] 2.3 Wire `verifySession` into `src/index.ts`
  ...
  - [x] 2.4 Tier 1 tests for `verifySession`
...
- [x] 3. AgentAdapter interface + KiroAdapter
  - [x] 3.1 Create `src/agent-adapter.ts`
  ...
  - [x] 3.2 Create `src/adapters/kiro.ts`
  ...
  - [x] 3.3 Refactor `src/index.ts` to use `KiroAdapter` (via adapter shim)
  ...
  - [x] 3.4 Update tier 2 test setup
  ...
  - [x] 3.5 Create `docs/kiro-hooks.md`
  ...
  - [x] 3.6 Tier 1 test for `KiroAdapter`
    - `name === 'kiro'`.
...
- [x] 4. ACP-layer fallback triggers
  - [x] 4.1 Wire post-`sendPrompt` → verifySession in `session-mgr.injectPrompt`
  ...
  - [x] 4.2 Wire inactivity-watchdog → verifySession-first with GitHub-outage protection
    ...
    - Branch on `result.verified` and `result.reason` (exhaustive over all six variants):
      ...
      - `reason: 'already_verified'` → session is in a terminal state; `updateMeta` would throw for non-active sessions — skip the write, proceed to grace-period kill.
      - `reason: 'prs_still_open' | 'no_prs'` → genuine timeout; write `timeout_inactivity` (existing path).
      - `reason: 'unknown_session'` → session directory was reaped; `updateMeta` throws on missing `meta.json`. Skip the write, proceed to kill.
    - _Requirements: 5.2, 5.3_

  - [x] 4.3 Tier 2 test for ACP fallback
    ...
    - **Inactivity watchdog with GitHub error** → the watchdog does NOT write `timeout_inactivity`. Use a `failNextGetPullState` mode on the fake GitHubClient to simulate the outage. Assert the session stays `active` and the watchdog logs a warning.
    - **Inactivity watchdog with already-verified session** → the watchdog does NOT write `timeout_inactivity` (updateMeta would throw for non-active). Session terminates cleanly via kill without a state write.
    - _Requirements: 5.1, 5.2_

- [x] 5. Harden `merge_pr` and `GitHubClient` in `src/github.ts`
  - [x] 5.1 Extend `PullState` with `mergeCommitSha`
  ...
  - [x] 5.2 Add 405 → already-merged success path
  ...
  - [x] 5.4 Add per-request timeout via `AbortController`
  ...
  - [x] 5.5 Expose all knobs via `GitHubClientOptions`
  ...
  - [x] 5.6 Tier 1 tests for hardening
...
- [x] 6. Final validation
  ...
  - [x] 6.2 Verify no regression in existing surfaces
  ...
  - [x] 6.3 Manual validation step (record in PR description)
...
- [x]* 7. Post-merge follow-up tracking
  - [x]* 7.1 Open a backlog item for "implement real KiroAdapter.installHooks"
  ...
  - [x]* 7.2 Open a backlog item for "second-adapter implementation (Claude Code or OpenCode)"
